...
If txtInput.Text <> "" Then
    For a = 1 To MaxUsers
        If User(a).Status = "Playing" Then
            frmMain.wsk(a).SendData Chr(8) & vbCrLf & Chr$(0)
            frmMain.wsk(a).SendData Chr$(8) & "<Server Message> - " & txtInput.Text & vbCrLf & vbCrLf & Chr$(0)
            DoEvents
        End If
    Next a
...
'Spawn Drug Dealers/Druggies every Hour
   ...
   Call SpawnNPC(N_Hooker, 2) 'Spawn 2 Hookers
   Call SpawnNPC(N_LoanShark, 2) 'Spawn 2 Loan Sharks
   Call SpawnNPC(N_Bouncer, 2)
   Call SpawnNPC(N_Politician, 2)
   Call SpawnNPC(N_Pedestrian, 2)
   Call SpawnNPC(N_Whore, 2)
   Call SpawnNPC(N_MeterMaid, 2)
   Call SpawnNPC(N_FBI, 2)
   Call SpawnNPC(N_DEA, 2)
   Call SpawnNPC(N_ATF, 2)
   Call SpawnNPC(N_Taliban, 1)
...
For a = 0 To UBound(IPBan)
   If IPBan(a) = "" Then
      ...
      frmMain.wsk(lstUsers.ListIndex + 1).SendData Chr$(2) & "You have been kicked and banned from the server.  If you feel this action was unjust, email the server administrator at jimmyjames@writeme.com to resolve the issue." & vbCrLf & vbCrLf & Chr$(0)
      DoEvents
